...
void glfw_key_callback(GLFWwindow *window, int key, int scancode, int action, int mods)
{
    ...
    {
        textureMix += 0.1f;
        textureMix = std::clamp(textureMix, 0.0f, 1.0f);
    }
...
void process_input(GLFWwindow *window)
{
    ...
    // {
    //     textureMix += 0.1f;
    //     std::cout << "Texture Mix set to: " << textureMix << std::endl;
    // }
...
int main()
{
    ...
    Shader shader("exercises/4.3.exercise4/shader.vs.glsl", "exercises/4.3.exercise4/shader.fs.glsl");
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        if (textureMix != prev_mix)
        {
            prev_mix = textureMix;
            shader.setFloat("textureMix", textureMix);
            std::cout << "TextureMix set to: " << textureMix << std::endl;
        }
